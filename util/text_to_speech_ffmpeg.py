...
from util.bgaudio import BackgroundAudioGenerator
from util.gcs_bucket import upload_to_gcs, download_from_gcs, delete_from_gcs, get_storage_client
...
async def create_final_video_v2(video_path: str, bg_audio_category: str, response_body: dict, output_path: str, model_name, unique_id: str, add_bg_music : str):
    ...
    original_audio_path = f"temp/{unique_id}_original_audio.wav"
    (
        ffmpeg
        .input(video_path)
        .output(original_audio_path, acodec='pcm_s16le', ac=2, ar='44100')
        .overwrite_output()
    ...
    original_videos_audio = AudioFileClip(original_audio_path) if os.path.exists(original_audio_path) else None
    #check for whether video is mute or not
